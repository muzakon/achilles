...
export default class ImageGenerationTasksController {
  ...
  async create({ request, userId }: HttpContext): Promise<unknown> {
    const payload = await Validator.validateRequest(request, imageGenerationTaskValidator)
  ...
  async status({ request, userId }: HttpContext): Promise<unknown> {
    const payload = await Validator.validateRequest(request, imageGenerationTaskStatusValidator)
    return await this.imageGenerationTaskService.getTaskStatus(userId, payload)
  }

  async all({ userId }: HttpContext): Promise<unknown> {
    return await this.imageGenerationTaskService.getAll(userId)
  }
}
